...
import type { Member, SupplementRecord } from "@/types/index";
...
export default function MemberSupplementsPage() {
  ...
  const load = useCallback(async () => {
    const [mRes, rRes] = await Promise.all([fetch(`/api/members/${id}`), fetch(`/api/supplements?memberId=${id}`)]);
  ...
  function openEdit(r: SupplementRecord) {
    setEditing(r);
    form.setFieldsValue({
      date: dayjs(r.date),
      name: r.name,
  ...
  async function handleDelete(recId: string) {
    const res = await fetch(`/api/supplements/${recId}`, { method: "DELETE" });
  ...
  const columns = [
    ...
    { title: "Supplement", dataIndex: "name",       key: "name"         },
    ...
    { title: "", key: "actions", width: 80, render: (_: unknown, r: SupplementRecord) => (
  ...
  return (
    <div>
      <Breadcrumb style={{ marginBottom: 12 }} items={[{ title: <Link href="/members">Members</Link> }, { title: <Link href={`/members/${id}`}>{member?.name}</Link> }, { title: "Supplements" }]} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Button type="text" icon={<ArrowLeftOutlined />} onClick={() => router.push(`/members/${id}`)} />
          <Title level={3} style={{ margin: 0 }}>Supplements — {member?.name}</Title>
        </div>
      ...
      <Modal title={editing ? "Edit Supplement" : "Add Supplement"} open={modalOpen} onCancel={() => setModalOpen(false)} footer={null} destroyOnHidden>
        <Form form={form} layout="vertical" onFinish={handleSubmit} style={{ marginTop: 16 }}>
          <Form.Item name="date" label="Date" rules={[{ required: true }]}>
            <DatePicker style={{ width: "100%" }} />
          </Form.Item>
          <Form.Item name="name" label="Supplement Name" rules={[{ required: true, message: "Required" }]}>
            <Input placeholder="e.g. Vitamin D3" />
          </Form.Item>
          <Form.Item name="dosage" label="Dosage">
            <Input placeholder="e.g. 1000 IU" />
          </Form.Item>
          ...
          <Form.Item name="reason" label="Reason">
            <Input placeholder="Why taking this?" />
          </Form.Item>
