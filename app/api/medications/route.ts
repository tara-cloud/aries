...
export async function POST(req: NextRequest) {
  ...
  const { memberId, date, name, dosage, timesPerDay, endDate, frequency, reason, notes } = await req.json();
  ...
  const record = await prisma.medicationRecord.create({
    data: { memberId, date: new Date(date), name: name.trim(), dosage: dosage || null, timesPerDay: timesPerDay || null, endDate: endDate ? new Date(endDate) : null, frequency: frequency || null, reason: reason || null, notes: notes || null },
    include: { member: { select: { name: true } } },
  });
